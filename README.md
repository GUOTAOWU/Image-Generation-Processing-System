...
* **repoto.pdf**
    * プロジェクトの機能、設計、実装詳細、および評価を含む詳細なレポートドキュメントです。（詳細な内容については、このファイルを参照してください）。
* **コード (Code Folder)**
    * **backend/**
        * バックエンドのソースコードが格納されています（FastAPIロジック、APIエンドポイント定義など）。
    * **frontend/**
        * フロントエンドのソースコードが格納されています（StreamlitによるUI構築、インタラクション処理など）。
    * **pretrained/**
        * **modnet_photographic_portrait_matting.onnx**:
            * MODNetの事前学習済みモデルファイルです。人物の背景切り抜き推論に使用されます。
    * **sqlite-jdbc-3.49.1.0.jar**
        * SQLiteデータベース接続および操作に使用されるJDBCドライバライブラリです。
